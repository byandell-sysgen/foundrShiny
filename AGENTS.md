...
applications for analyzing multiparent founder study data. It is a
companion to the `foundr` package (core analysis functions).
...
The package is fully modularized using Shiny modules (Mastering Shiny
conventions). Every module in `R/` follows this naming pattern: -
`*Input()` — input UI - `*UI()` — parameter UI - `*Output()` —
output/display UI - `*Server()` — server logic - `*App()` — standalone
test app
...
Each panel delegates to sub-modules (e.g., `corTableApp.R`,
`volcanoApp.R`, `timePlotApp.R`).
...
1.  **`main_par`** — dataset selection, trait ordering, plot/table
    toggle, plot height
2.  **`panel_par`** — strain/genotype, sex (B/F/M/C), faceting, table
    type
3.  **`plot_par`** — order name, interaction settings, volcano
    thresholds (`volsd`, `volvert`), row names

## Runtime Data Objects

Loaded at app startup via
...
Example deployment apps live in `inst/shinyApp/` and in external
directories (e.g., `FounderDietStudy/deployNew/app.R`). Typical setup:
